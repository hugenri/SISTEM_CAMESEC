...
$estatusFactura = DataSanitizer::sanitize_input($_POST['estatusFactura']);

$data = [$idFactura, $estatusFactura];
...
$response = DataValidator::validateLettersOnly($estatusFactura, $messageLength);
if ($response !== true) {
    $respuesta_json->response_json($response);
...
$parametros = array(
  ...
  $consulta = ConsultaBaseDatos::ejecutarConsulta($sql, $parametros);
    if ($consulta) {
        $response = array('success' => true, 'message' => 'Se actualizo el estodo de la factura a: '. $estatusFactura); // Encerrar la cotización en un array
        $respuesta_json->response_json($response);
    } else {
        $response = array('success' => false, 'message' => 'No se actualizo el estado de la factura');
        $respuesta_json->response_json($response);
    }
