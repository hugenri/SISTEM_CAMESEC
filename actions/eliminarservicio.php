...
if($id == ""){ //para verificar que los datos enviados por POST tenga un valor.
	...
	$response = array('success' => false, 'message' => 'Faltan datos');
	      ...
	      $result = $consulta->deleteService($id); //se realiza la consulta SQL a MYSQL
	  
	if($result == true){//si elimino
	   $response = array('success' => true, 'message' => 'Servicio  eliminado!');
	   
	}else{
		$response = array('success' => false, 'message' => 'El Servicio no se pudo eliminar');
	}
}
