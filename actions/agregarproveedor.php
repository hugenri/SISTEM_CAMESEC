  ...
  $apellidoM = DataSanitizer::sanitize_input($_POST['apellidoMaterno']);
   ...
   if ($response !== true) {
    $validacion = false;
      ...
      $result = $consulta->providerExists($email);
          if($result == true){
            $validacion == false;
      ...
      if($validacion == true){//Si devuelve true, significa que el reCAPTCHA es válido y se puede continuar con el procesamiento del formulario
         $result = $consulta->createProvider($razonSocial, $nombre, $apellidoP, $apellidoM, $infoContacto, $calle, $numero, $colonia, 
         $municipio, $estado, $cp, $email, $telefono, $otrosDetalles);

         if($result === true){
               $response = array("success" => true, 'message' => 'Proveedor registrado con exito!');
         }else{
