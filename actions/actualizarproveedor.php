...
$calle, $numero, $colonia,$municipio, $estado, $cp, $otrosDetalles, $categoria];
...
if(DataValidator::validateVariables($data) === false){
  ...
  $datos = [$nombre. $apellidoP, $apellidoM, $municipio, $estado, $categoria];
   ...
   if ($response !== true) {
    $validacion = false;
     ...
     if($validacion == true){//Si devuelve true, significa que el reCAPTCHA es válido y se puede continuar con el procesamiento del formulario
        $result = $consulta->updateProvider($id, $razonSocial, $nombre, $apellidoP, $apellidoM, 
