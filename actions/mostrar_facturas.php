...
if($action == "mostar_facturas"){

$sql = "SELECT DISTINCT
f.idFactura, f.fecha AS fecha_factura,
...
$parametros = array(
  ...
  $datos = ConsultaBaseDatos::ejecutarConsulta($sql, $parametros, true);
    if (!empty($datos)) {
        $response = array('success' => true, 'facturas' => $datos); // Encerrar la cotización en un array
        $respuesta_json->response_json($response);
    } else {
        $response = array('success' => false, 'message' => 'No hay facturas');
        $respuesta_json->response_json($response);
    }
