...
if (DataValidator::validateVariables($data) === false) {
   ...
   if ($response !== true) {
    ...
    if ($validacion == true) { // Si es true, se puede continuar
        $result = $consulta->createRequisicion($fecha, $descripcion, $cantidad, $precioUnitario,
                                             $importeTotal, $observaciones, $idProveedor, 
                                              $idServicio, $idConcepto, $idCotizaciones, 
                                              $idProducto, $idCliente, $idCatalogoRequisicion);

        if ($result === true) {
            $response = array("success" => true, 'message' => 'Requisición registrada con éxito!');
        } else {
